...
![MEGA Viewer: папки и файлы в общей сетке](docs/screenshot1.png)
...
2. Откройте **MEGA-Viewer-Setup-1.2.3.exe** и выберите папку установки.
...
- Сверху показан кликабельный путь вида **Мои коллекции › Медиатека › Фото**. Нажмите название нужной родительской папки, чтобы вернуться к ней. Текущая папка выделена жирным. Отдельная стрелка возврата убрана.
- Нажатие на запись в **«Недавних»** открывает главную папку публичной ссылки, сбрасывает поиск и включает «Все файлы». Это работает и для ранее сохранённых ссылок на вложенные файлы или папки.
- При открытии папки поисковый запрос и фильтр сбрасываются на «Все файлы», чтобы сразу показывать её содержимое.
- При вставке ссылки вручную по-прежнему открывается указанная в ней вложенная папка; к корню можно перейти через путь сверху.
- Скриншот в README обновлён под новый интерфейс.
...
Путь над каталогом позволяет перейти к любой родительской папке нажатием на её название. Поиск работает в текущей папке и всех вложенных. Переход в другую папку очищает поиск и включает «Все файлы». Нажатие на запись в «Недавних» возвращает к корню этой публичной ссылки; для отдельной ссылки на файл открывается сам файл.
...
| Открыть главную папку сохранённой ссылки | Нажать на ссылку в «Недавних» слева |
...
| По ссылке открылись только видео, а нужны соседние папки | Нажмите название корня в пути над каталогом или откройте эту ссылку из «Недавних». |
